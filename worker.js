...
const CONFIG = {
  backendUrls: [
    "https://api.v1.mk/sub",          // 对 Hy2 支持最稳的后端
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (AIRPORT_URLS.length > 0) {
      for (const backend of CONFIG.backendUrls) {
        const batchPromises = AIRPORT_URLS.map(async (subUrl) => {
          // target=mihomo 确保后端输出 Hysteria 2 格式
        ...
        if (successInBackend && allProxyBlocks.length > 0) break;
      }
    }
    ...
    // 4. 生成 YAML (1:1 还原原始代码所有配置，绝无删减)
    const trafficHeader = `# 📊 流量: ${((totalUpload+totalDownload)/(1024**3)).toFixed(1)}GB / ${(summary.total/(1024**3)).toFixed(1)}GB | 🏆 Hy2 修复版`;
...
proxies:
${nodes.length > 0 ? nodes.join("\n") : "  - {name: \"⚠️ 抓取失败\", type: ss, server: 127.0.0.1, port: 80, cipher: aes-128-gcm, password: \"pw\"}"}
